...
export default function DocumentPage() {
    ...
    const { suggestion, requestSuggestion, acceptSuggestion, rejectSuggestion, editSuggestion } =
    ...
    const titleMutation = useMutation({
        ...
        onSuccess: () => qc.invalidateQueries({ queryKey: ['document', params.id] }),
    });
    ...
    useEffect(() => {
        ...
        return () => { socket.off('ai:committed', handleCommitted); };
    }, [socket]);

    // Keyboard shortcut: Cmd+Enter to accept
    useEffect(() => {
        function handleKeyDown(e: KeyboardEvent) {
            if ((e.metaKey || e.ctrlKey) && e.key === 'Enter' && suggestion?.status === 'pending') {
