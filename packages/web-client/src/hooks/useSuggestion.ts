...
export function useSuggestion(
    ...
    useEffect(() => {
        ...
        function handleStream({ token, suggestionId }: { token: string; suggestionId: string }) {
            setSuggestion((prev) => {
        ...
        function handleComplete({ suggestionId, text }: { suggestionId: string; text: string }) {
            setSuggestion((prev) =>
    ...
    const requestSuggestion = useCallback(
        (promptType: string, context: string) => {
            ...
            setSuggestion({ id: tempId, text: '', status: 'streaming', requestedBy: currentUserId });
    ...
    return { suggestion, requestSuggestion, acceptSuggestion, rejectSuggestion, editSuggestion };
}
