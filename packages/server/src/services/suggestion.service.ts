    ...
    const stream = await client.messages.stream(
      {
        ...
        max_tokens: 1024,
        system:
          'You are a collaborative writing assistant. Provide focused suggestions. Just provide the suggestion text directly without commentary.',
        messages: [{ role: 'user', content: buildPrompt(promptType, context) }],
      },
      { signal: abort.signal },
    );
